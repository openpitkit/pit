...
venv_dir := env_var_or_default("PIT_VIRTUAL_ENV", repo_dir / ".venv")
...
install: ensure-node
    ...
    if rustup target list --installed | grep -qx wasm32-unknown-unknown; then
      ...
      echo "wasm32-unknown-unknown: installing"
      rustup target add wasm32-unknown-unknown
    fi

    # 2. wasm-bindgen-cli at the exact version the crate resolved to. Derive
    # it dynamically (cargo metadata, with a Cargo.lock fallback) so it always
    # matches the `wasm-bindgen` pin in bindings/js/Cargo.toml.
    WB_VERSION="$(
    ...
    if [[ -z "${WB_VERSION}" ]]; then
      WB_VERSION="$(
        awk '/^name = "wasm-bindgen"$/ { f = 1; next }
             f && /^version = / { gsub(/[",]/, "", $3); print $3; exit }' \
          Cargo.lock
      )"
    fi
    if [[ -z "${WB_VERSION}" ]]; then
      echo "error: could not resolve the wasm-bindgen version" >&2
      exit 1
    fi
    if command -v wasm-bindgen >/dev/null 2>&1 \
      && [[ "$(wasm-bindgen --version | awk '{print $2}')" == "${WB_VERSION}" ]]; then
      echo "wasm-bindgen-cli ${WB_VERSION}: already installed"
    else
      echo "wasm-bindgen-cli ${WB_VERSION}: installing"
      # Prefer the fast prebuilt path; fall back to a source build.
      cargo binstall -y "wasm-bindgen-cli@${WB_VERSION}" \
    ...
    if command -v wasm-opt >/dev/null 2>&1; then
      ...
      echo "wasm-opt: optional, not found (enables release -Oz)."
      echo "  install with: brew install binaryen  # or: apt-get install binaryen"
    fi

# Provisioning JS/WASM tooling is Unix-only for now.
[windows]
install:
    @echo just install is Unix-only; install JS/WASM tooling manually and run just python-develop-debug
    exit /b 1
...
run-examples-debug: run-examples-go-debug run-examples-python-debug run-examples-cpp-debug run-examples-js

# Run all examples in release mode.
run-examples-release: run-examples-go-release run-examples-python-release run-examples-cpp-release run-examples-js
...
lint-js: build-js
    ...
    cd bindings/js && npm run lint
    cd bindings/js && npm run format:check
...
# Full Go debug test suite including race instrumentation.
test-go-debug-suite: test-go-debug test-go-race
...
# Convenience alias for the single supported JS/WASM test mode.
...
# Mode-neutral alias: WASM has one build/runtime mode for all aggregate checks.
run-examples-js: run-examples-js-debug
...
run-examples-js-table test_file="examples/tables/spot/coverage.md":
    just run-examples-js-table-debug "{{ test_file }}"

# Lint and test JS (non-mutating).
check-js-dry-debug: lint-js test-js-debug

# Convenience alias for the single supported JS/WASM check mode.
check-js: check-js-dry-debug
...
lint-cpp: build-cpp-debug build-examples-cpp-debug
    ...
    find bindings/cpp/test -type f -name '*.cpp' -print0 | xargs -0 clang-tidy -quiet "${tidy_args[@]}" --config-file bindings/cpp/.clang-tidy -p bindings/cpp/build-debug --line-filter="$header_filter"
    cmake -S examples/cpp -B examples/cpp/build-debug -DCMAKE_BUILD_TYPE=Debug -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
    find examples/cpp -type f -name '*.cpp' -not -path '*/build/*' -not -path '*/build-*/*' -not -path '*/test/*' -not -name '*_test.cpp' -print0 | xargs -0 clang-tidy -quiet "${tidy_args[@]}" --config-file bindings/cpp/.clang-tidy -p examples/cpp/build-debug
[windows]
lint-cpp: build-cpp-debug build-examples-cpp-debug
