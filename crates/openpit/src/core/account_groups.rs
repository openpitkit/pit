...
/// Public handle to the engine's account-group registry.
///
/// Obtained from [`Engine::accounts`](crate::Engine::accounts). Cloneable;
/// every clone refers to the same registry. An account belongs to at most one
/// group at a time.
///
/// # Thread-safety
///
/// `Accounts` inherits the engine's synchronization mode through its inner
/// handle; see [`Engine`](crate::Engine)'s threading section for the contract.
///
/// # Examples
///
/// ```rust
...
where
    StorageFactory: storage::LockingPolicyFactory + storage::CreateStorageFor<AccountId> + 'static,
{
    pub(crate) fn new(handle: AccountGroupsHandle<StorageFactory>) -> Self {
        Self { handle }
    }
