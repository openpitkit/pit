...
use crate::core::{
    AccountAdjustmentContext, AccountControl, AccountOutcomeEntry, HasAccountAdjustmentBalance,
...
{
    fn with_adjustment_state_writer<R>(
        &self,
        ...
        let owns_pnl_lease = self
            ...
            .unwrap_or(false);
        if owns_pnl_lease {
            // A prior batch element can retain this account's PnL lease until
            // finalization. If a transition queues while an execution reader
            // waits for that lease, the batch must finish through the same
            // bypass as rollback or all three operations deadlock.
            ctx.with_state_writer_bypassing_transition(operation)
        } else {
            ctx.with_state_writer(operation)
        }
    }
        ...
        <<Sync as SyncMode>::StorageLockingPolicyFactory as crate::storage::LockingPolicyFactory>::Policy: 'static,
    {
        ...
        let operation = || {
            ...
            let update = self.holdings.with_mut_or_insert(
                key.clone(),
                Holdings::zero,
                |slot, _is_new| -> Result<AdjustmentSlotUpdate, Rejects> {
                    ...
                    if let Some(state) = request.balance_realized_pnl {
                        let state = match state {
                            PnlState::Value(value) => PositionPnlState::Pnl(value),
                            PnlState::Halted(reason) => PositionPnlState::Halted(reason),
                        };
                        new = new.force_set_realized_pnl_state(state);
                    }

                    let halt_clears_average = matches!(
                        request.balance_realized_pnl,
                        Some(PnlState::Halted(_))
                    );
        ...
        self.with_adjustment_state_writer(ctx, account_id, owner_id, operation)
    }
        ...
        <<Sync as SyncMode>::StorageLockingPolicyFactory as crate::storage::LockingPolicyFactory>::Policy: 'static,
    {
        ...
        self.with_adjustment_state_writer(ctx, account_id, owner_id, || {
            ...
            let previous =
                self.pnl
                    .with_mut(account_id, super::AccountPnlEntry::zero, |entry, _| {
                        let previous = *entry;
